/**
 * migrate-email-domain.js — move every user from @gyftr.net to @gyftr.com.
 *
 ...
 * own password on first login — which also satisfies the forced-password-change
...
async function main() {
  ...
  if (!toMigrate.length) {
    console.log(`No accounts on @${FROM} — nothing to do.`);
    await pool.end();
    return;
  }
  ...
  for (const u of toMigrate) {
    ...
    process.stdout.write(`  ${u.email.padEnd(34)} → ${newEmail.padEnd(34)}`);
    ...
    try {
      ...
      // Temporary → forced password change on first login.
  ...
  if (DRY_RUN) {
    ...
    // users.email is the primary key — a row already sitting on the new
    ...
    if (collisions.length) {
      console.error('  ABORTED — these directory rows already exist on the new domain:');
      collisions.forEach(c => console.error(`    ${c.old_email} → ${c.new_email} (already present)`));
      console.error('  Remove the duplicate rows in Admin → Team Directory, then re-run.');
      await pool.end();
      process.exit(1);
    }
  ...
  console.log(`\nCognito — created: ${created}  skipped: ${skipped}  failed: ${failed}`);
  if (!DRY_RUN) {
    ...
    if (!DELETE_OLD) console.log(`\nOld @${FROM} accounts are disabled, not deleted — re-enable in the`);
